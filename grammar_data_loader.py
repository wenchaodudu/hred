...
class Dataset(data.Dataset):
    ...
    def __init__(self, src_path, parse_path, word2id):
        ...
        self.slot_seqs = [None for x in range(self.num_total_seqs)]
        self.max_len = 100
        ...
        for x in range(self.num_total_seqs):
            ...
            self.trg_seqs[x] = self.preprocess_parsed(parse_file[x]['parse'], self.word2id)
            self.slot_seqs[x] = None
            if 'acts' in parse_file[x]:
                slot_seq = []
                for act in parse_file[x]['acts']:
                    slot_seq.append(('NT-{}'.format(act['name']), None))
                    for name, val in act['vals']:
                        if val is not None:
                            slot_seq.append(('NT-SLOT-{}'.format(name), val))
                self.slot_seqs[x] = slot_seq

    def __getitem__(self, index):
        """Returns one data pair (source and target)."""
        src_seq = self.src_seqs[index]
        trg_seq = self.trg_seqs[index]
        slot_seq = self.slot_seqs[index]
        return src_seq, trg_seq, slot_seq
...
def collate_fn(data):
    ...
    src_seqs, trg_seqs, slots = zip(*data)
    data = list(zip(src_seqs, trg_seqs, [*range(len(data))]))
    # sort a list by sequence length (descending order) to use pack_padded_sequence
    data.sort(key=lambda x: len(x[0]), reverse=True)

    # seperate source and target sequences
    src_seqs, trg_seqs, indices = zip(*data)

    # merge sequences (from tuple of 1D tensor to 2D tensor)
    src_seqs, src_lengths = merge(src_seqs)
    trg_seqs, trg_lengths = merge(trg_seqs)

    return src_seqs, src_lengths, trg_seqs, trg_lengths, indices, slots


def get_loader(src_path, parse_path, word2id, batch_size=100, shuffle=True):
    ...
    dataset = Dataset(src_path, parse_path, word2id)
