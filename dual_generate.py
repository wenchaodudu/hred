...
def main(config):
    ...
    parse_dict = json.load(open('./{}.pos.dictionary.json'.format(config.data)))
    vocab_size = len(dictionary)
    word_embedding_dim = 300
    print("Vocabulary size:", len(dictionary))

    if config.attn:
        test_loader = get_dl_loader('./data/{}.test.src'.format(config.data), './data/{}.test.pos.trg'.format(config.data), dictionary, parse_dict, 20, shuffle=False)
    else:
        test_loader = get_hr_loader('./data/{}.test.src'.format(config.data), './data/{}.test.trg'.format(config.data), dictionary, 40, shuffle=False)
    ...
    for _, batch in enumerate(test_loader):
        if config.attn:
            ...
            src_seqs, src_lengths, src_indices, ctc_seqs, ctc_lengths, ctc_indices, trg_seqs, trg_lengths, trg_indices, turn_len = batch
        ...
        '''
        if config.attn:
            responses, nll = hred.generate(src_seqs, src_lengths, indices, max_len, 10, 10)
        else:
