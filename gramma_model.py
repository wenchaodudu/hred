...
class LexicalizedGrammarDecoder(nn.Module):
    ...
    def forward(self, src_seqs, src_lengths, indices, trg_seqs, trg_lengths, parent_seqs, sibling_seqs, leaf_seqs, rule_seqs):
        batch_size = src_seqs.size(0)
    ...
    def loss(self, src_seqs, src_lengths, indices, trg_seqs, trg_lengths, parent_seqs, sibling_seqs, leaf_seqs, rule_seqs, word_mask, rule_mask):
        decoder_outputs = self.forward(src_seqs, src_lengths, indices, trg_seqs, trg_lengths, parent_seqs, sibling_seqs, leaf_seqs, rule_seqs)
        ...
        nt_loss = None
        if self.lex_level > 0:
            nts = self.nt_dist(self.nt_out(decoder_outputs))
    ...
    def generate(self, src_seqs, src_lengths, indices, max_len, beam_size, top_k):
        ...
        if self.lex_level == 0:
            ...
            self.nt_cand_num = 1
            nt_logits = self.nt_dist(self.nt_out(decoder_output))
        ...
        for t in range(max_len-1):
            ...
            # keep only one copy if nothing is generated
