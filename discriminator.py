...
class HRED(nn.Module):
    def __init__(self, dictionary, vocab_size, dim_embedding, init_embedding, hidden_size):
        ...
        self.u_encoder = UtteranceEncoder(dim_embedding, hidden_size, 'lstm', 0.2)
        self.uu_encoder = UtteranceEncoder(dim_embedding, hidden_size, 'lstm', 0.2)
        self.cenc_input_size = hidden_size
        self.c_encoder = ContextEncoder(self.cenc_input_size, hidden_size, type='gru')
        self.hidden_size = hidden_size
        '''
    ...
    def loss(self, src_seqs, src_lengths, indices, ctc_seqs, ctc_lengths, ctc_indices, trg_seqs, trg_lengths, trg_indices, turn_len, labels):
        ...
        #return loss.sum() / _batch_size
        return loss

    def evaluate(self, src_seqs, src_lengths, indices, ctc_seqs, ctc_lengths, ctc_indices, trg_seqs, trg_lengths, trg_indices, turn_len, labels):
        ctc_seqs = self.embedding(Variable(ctc_seqs.cuda()))
        ctc_packed_input = pack_padded_sequence(ctc_seqs, ctc_lengths, batch_first=True)
        ctc_output = self.u_encoder(ctc_packed_input)
        ctc_output = ctc_output[torch.from_numpy(np.argsort(ctc_indices)).cuda()]
...
class LSTM(nn.Module):
    ...
    def loss(self, src_seqs, src_lengths, src_indices, trg_seqs, trg_lengths, trg_indices, labels):
        src_seqs = self.embedding(Variable(src_seqs.cuda()))
        trg_seqs = self.embedding(Variable(trg_seqs.cuda()))
