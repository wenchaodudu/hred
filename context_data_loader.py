...
class Dataset(data.Dataset):
    ...
    def __init__(self, src_path, trg_path, context_path, word2id):
        """Reads source and target sequences from txt files."""
        self.src_seqs = open(src_path).readlines()
        is_parse = trg_path.find('parse') > -1 or trg_path.find('rules') > -1
        if is_parse:
            parse_file = pickle.load(open(trg_path, 'rb'))
            self.trg_seqs = [None for x in range(len(self.src_seqs))]
        else:
...
def collate_fn(data):
    ...
    src_seqs, trg_seqs, ctc_seqs = zip(*data)
    data = list(zip(src_seqs, trg_seqs, ctc_seqs, [*range(len(data))]))
    # sort a list by sequence length (descending order) to use pack_padded_sequence
    data.sort(key=lambda x: len(x[0]), reverse=True)

    # seperate source and target sequences
    src_seqs, trg_seqs, ctc_seqs, indices = zip(*data)
    ...
    return src_seqs, src_lengths, trg_seqs, trg_lengths, ctc_seqs, ctc_len, indices


def get_ctc_loader(src_path, trg_path, context_path, word2id, batch_size=100, shuffle=True):
    ...
    dataset = Dataset(src_path, trg_path, context_path, word2id)
